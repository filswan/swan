...
def send_deals(config_path, miner_id, task_name=None, metadata_csv_path=None, deal_list=None):
    ...
    if deal_list:
        send_deals_to_miner(deal_config, output_dir, task_name=task_name, deal_list=deal_list)
    elif metadata_csv_path:
        send_deals_to_miner(deal_config, output_dir, csv_file_path=metadata_csv_path)
    else:
