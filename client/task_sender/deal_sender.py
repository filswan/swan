...
def send_deals(config_path, miner_id, task_name=None, metadata_csv_path=None, deal_list=None, task_uuid=None, out_dir=None):
    ...
    manually_confirm = config['sender']['manually_confirm']
    output_dir = out_dir
    if not out_dir:
    ...
    if deal_list:
        return send_deals_to_miner(deal_config, output_dir, manually_confirm, task_name=task_name, deal_list=deal_list, task_uuid=task_uuid)
    elif metadata_csv_path:
        return send_deals_to_miner(deal_config, output_dir, manually_confirm, csv_file_path=metadata_csv_path, task_uuid=task_uuid)
    else:
